...
final class QRTLSimulation: ObservableObject {
    ...
    private func updateLattice(dt: Double) {
        ...
        //
        // The SceneKit frame interval is only for visualization.
        // The lattice itself evolves using its physical timestep.
        // ============================================================
        ...
        // ============================================================
        // ANALYTIC OSCILLATOR STEP
        //
        // x(t+h) = x cos(ωh) + v/ω sin(ωh)
        // v(t+h) = v cos(ωh) - ωx sin(ωh)
        //
        // This is much more stable than Euler integration for the
        // extremely stiff QRTL lattice.
        // ============================================================
        ...
        // LIMITS
        //
        // These limits are applied only after the stable oscillator
        // step. They therefore do not destroy the oscillator state
        // every frame.
        // ============================================================
        ...
        let maxVelocity =
            max(
                Double(QRTLConstants.maxLatticeVelocity),
                1.0e-12
            )
        ...
        for index in cells.indices {
            ...
            if !cell.neighbors.isEmpty {
                ...
                for neighborIndex in cell.neighbors {
                    ...
                    neighborDisplacement +=
                        SIMD3<Double>(
                            ...
                            Double(neighbor.displacement.y),
                            Double(neighbor.displacement.z)
                        )
                ...
                if count > 0.0 {
                    ...
                    neighborDisplacement /= count

                    neighborTwist /= count

                    neighborStrain /= count
                }
            }

            // --------------------------------------------------------
            // Current mechanical state in Double precision.
            // --------------------------------------------------------
            ...
            // Neighbor coupling
            //
            // Apply coupling as a small velocity kick AFTER the
            // analytic self-oscillation.
            //
            // Importantly, do not multiply the coupling by k/m.
            // The coupling coefficient is treated as a dimensionless
            // lattice coupling in the normalized lattice dynamics.
            // --------------------------------------------------------
            ...
            let neighborDelta =
                neighborDisplacement - newX

            let couplingKick =
                neighborDelta *
                (coupling / max(h, 1.0e-30)) *
                0.01

            // The raw physical rate can be enormous, so normalize
            // the coupling kick to the lattice velocity scale.
            let normalizedKick =
                couplingKick /
                max(
                    omega,
            ...
            let direction =
                simd_length(cell.modeDirection) > 0.0001
                ? cell.modeDirection
                : SIMD3<Float>(1, 0, 0)
            ...
            // Phase
            //
            // Use the oscillator state directly rather than
            // integrating an almost-zero phase increment.
            // --------------------------------------------------------
            ...
            // Amplitude
            //
            // This is the harmonic-oscillator amplitude.
            // --------------------------------------------------------
            ...
            // Mechanical energy
            //
            // Convert the lattice displacement and velocity to SI
            // units before calculating energy.
            // --------------------------------------------------------
            ...
            // These are calculated after the mechanical state
            // has been updated.
        ...
        // ============================================================
        // COMMIT ALL CELLS SIMULTANEOUSLY
        // ============================================================
